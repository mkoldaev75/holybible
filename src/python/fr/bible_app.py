...
# DB_PATH = resource_path("fr_bible.db")
DB_PATH = "../../sqlite/fr_bible.db"
print("DB_PATH =", DB_PATH) 

class BibleApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Bible fr (SQLite)")
        self.resize(800, 600)
    ...
    def load_books(self):
        self.cursor.execute("SELECT _id, biblename FROM frbible ORDER BY _id")
    ...
    def load_chapters(self):
        ...
        if book_id is None:
            return
        self.cursor.execute("SELECT chapters FROM frbible WHERE _id = ?", (book_id,))
