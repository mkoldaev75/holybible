...
# DB_PATH = resource_path("cs_bible.db")
DB_PATH = "../../sqlite/cs_bible.db"
print("DB_PATH =", DB_PATH) 

class BibleApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Bible cs (SQLite)")
        self.resize(800, 600)
    ...
    def load_books(self):
        self.cursor.execute("SELECT _id, biblename FROM csbible ORDER BY _id")
    ...
    def load_chapters(self):
        ...
        if book_id is None:
            return
        self.cursor.execute("SELECT chapters FROM csbible WHERE _id = ?", (book_id,))
