...
# массив кодов языков
langs=(
af am ara be bg bn cs da de du en es et fa fi fr gr heb hin hr hu hy is it jap ka kk ko la lt lv mk no pl pt ro ru sk sq sr sv sw tam th tl tr uk ur vi zh
)
...
for lang in "${langs[@]}"; do
    ...
    DB_FILE="$SQLITE_DIR/${lang}_bible.db"
    if [ ! -f "$DB_FILE" ]; then
        echo "⚠️  Предупреждение: база данных $DB_FILE не найдена!"
    fi

    # создаём bible_app.py
    cat > "$TARGET_DIR/bible_app.py" <<EOF
import sys
import sqlite3
...
# DB_PATH = resource_path("${lang}_bible.db")
DB_PATH = "../../sqlite/${lang}_bible.db"
print("DB_PATH =", DB_PATH) 

class BibleApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Bible ${lang} (SQLite)")
        self.resize(800, 600)
    ...
    def load_books(self):
        self.cursor.execute("SELECT _id, biblename FROM ${lang}bible ORDER BY _id")
    ...
    def load_chapters(self):
        ...
        if book_id is None:
            return
        self.cursor.execute("SELECT chapters FROM ${lang}bible WHERE _id = ?", (book_id,))
...
EOF
    ...
    echo "PyQt5>=5.15.9" > "$TARGET_DIR/requirements.txt"

    echo "✅ Создано: $TARGET_DIR"
done
